...
public class PlatoDirectoServlet extends HttpServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			BusinessDelegate.getInstancia().crearPlatoDirecto(rubro, caducidad, comisionMozo, fecha, precio, nombreArea, idInsumo, cantInsumo);
			response.sendRedirect("/restaurant/plato-directo-crear");
		} catch (BaseDeDatosException | ConexionException | RubroNoExisteException | InsumoNoExisteException | AreaNoExisteException e) {
			response.sendError(500, e.getMessage());
		}
	}
